...
def compute_roc_curve(labels, scores, num_thresholds=100):
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.test:
            ...
            y_true = np.array(val_metrics["all_labels"])
            y_scores = np.array(val_metrics["all_probs"])
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(args.output_dir, "roc_curve_10.png"))

            logging.info("Saved ROC curve to roc_curve_10.png")
